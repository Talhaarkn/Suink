...
export function CreateQuizPage() {
  ...
  const [quizDuration, setQuizDuration] = useState(1) // Quiz süresi (dakika) - default 1 dakika test için
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Create quiz on blockchain (real signature)
      ...
      const { digest: txDigest, quizObjectId } = await quizContractService.createQuiz(
        quizTitle,
        quizDescription,
        contractQuestions,
        quizConfig,
      ...
      console.log('✅ Quiz creation transaction completed')
      console.log('📄 Transaction digest:', txDigest)
      console.log('🆔 Quiz object ID:', quizObjectId)

      // Temporary: Relax validation for debugging
      if (!quizObjectId) {
        throw new Error('Quiz oluşturulamadı: Blockchain object ID alınamadı. Lütfen tekrar deneyin.')
      }
      
      // Warn if not a proper Sui object ID
      if (!quizObjectId.startsWith('0x') || quizObjectId.length < 10) {
        console.warn('⚠️ Quiz ID is not a standard Sui object ID:', quizObjectId)
        console.warn('⚠️ This may be a debug fallback ID')
      }
      
      const finalQuizId = quizObjectId
      
      const newQuiz = {
        id: finalQuizId, // Use real blockchain object ID or generated ID
        title: quizTitle,
        ...
        prizePool,
        duration: quizDuration, // Quiz süresi (dakika)
        endTime: new Date(Date.now() + quizDuration * 60 * 1000).toISOString(), // Bitiş zamanı
        ...
        blockchainObjectId: quizObjectId // Real blockchain object ID
      }

      console.log('🔍 Quiz Creation Debug:')
      console.log('📋 Created quiz object:', newQuiz)
      console.log('🆔 Final Quiz ID:', finalQuizId)
      console.log('⛓️ Blockchain QuizObjectId:', quizObjectId)
      console.log('✅ Is valid Sui object ID?', /^0x[a-fA-F0-9]{64}$/.test(finalQuizId))
      console.log('📊 Transaction result:', { digest: txDigest, quizObjectId })
      
      // If we got a valid blockchain object ID, use it
      if (quizObjectId && /^0x[a-fA-F0-9]{64}$/.test(quizObjectId)) {
        console.log('✅ Using blockchain object ID:', quizObjectId)
        // Update the quiz object with the real blockchain ID
        newQuiz.id = quizObjectId
        newQuiz.blockchainObjectId = quizObjectId
      } else {
        console.log('❌ No valid blockchain object ID, using fallback:', finalQuizId)
      }
      ...
      if (timeLockDuration > 0 || privacyEnabled || multiSigEnabled || whitelistAddresses.length > 0) {
        ...
        try {
          ...
          console.log('✅ Seal protection applied:', sealId)
        } catch (sealError) {
          console.warn('⚠️ Seal protection failed, continuing without protection:', sealError)
      ...
      console.log('💾 Saving quiz to database:', {
        id: newQuiz.id,
        title: newQuiz.title,
        idType: newQuiz.id.startsWith('0x') ? 'blockchain' : 
                newQuiz.id.startsWith('debug_') ? 'debug' : 
                newQuiz.id.startsWith('quiz_') ? 'legacy' : 'unknown'
      })
      
      const saved = await walrusDatabaseService.saveQuiz(newQuiz)
      if (saved) {
        console.log('✅ Quiz saved to Walrus database')
      } else {
        console.log('⚠️ Quiz saved to localStorage (Walrus unavailable)')
      }
      ...
      if (storedQuizzes) {
        ...
        const foundQuiz = allQuizzes.find((q: any) => q.id === newQuiz.id)
        console.log('🔍 Quiz verification:', foundQuiz ? '✅ Found in storage' : '❌ Not found in storage')
      }
      ...
      // Show quiz ID and navigate to home to see the quiz
      setCreatedQuizId(finalQuizId)
      
      // Navigate to home page to see the created quiz
      setTimeout(() => {
        navigate('/')
      }, 2000) // 2 saniye sonra home'a git
      ...
      if (error instanceof Error) {
        if (error.message.includes('Insufficient gas')) {
          errorMessage = 'Insufficient gas to create quiz. Please ensure you have enough SUI in your wallet.'
        } else if (error.message.includes('Transaction failed')) {
          errorMessage = 'Blockchain transaction failed. Please check your wallet connection and try again.'
        } else if (error.message.includes('User rejected')) {
          errorMessage = 'Transaction was cancelled. Please try again if you want to create the quiz.'
        } else {
          errorMessage = `Quiz creation failed: ${error.message}`
        }
      }
        ...
        <div className="quiz-card">
          ...
          <div className="bg-sui-light-blue/10 border border-sui-light-blue/20 rounded-lg p-4 mb-6">
            ...
            <p className="text-white/60 text-sm mb-3">
              Share this Quiz ID with others so they can join your quiz using the "Quiz ID ile Katıl" feature on the homepage.
            </p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value={1} className="bg-gray-800 text-white">1 minute</option>
            ...
            <p className="text-white/60 text-sm mt-2">
              Quiz will automatically end after this time. Winners get 70% of prize pool, all participants get 30%.
            </p>
          </div>
        </div>
      </div>
